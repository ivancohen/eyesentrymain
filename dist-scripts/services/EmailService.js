...
const sendEmailViaEdgeFunction = async (recipientEmail, subject, htmlContent, fromAddress) => {
    try {
...
const sendEmailDirectly = async (recipientEmail, subject, htmlContent, fromAddress = 'EyeSentry <no-reply@eyesentry.com>') => {
    try {
...
const generateSpecialistEmailContent = (accessCode, patientName, doctorName) => {
...
export const EmailService = {
    async sendSpecialistAccessLink(recipientEmail, accessCode, patientName, doctorName) {
        ...
        try {
            ...
            if (isDevelopment) {
                ...
                return await sendEmailDirectly(recipientEmail, subject, htmlContent, 'EyeSentry <no-reply@eyesentry.com>');
            }
            else {
                // In production mode, use the Edge Function
                console.log('Production mode detected, using Edge Function for email');
                return await sendEmailViaEdgeFunction(recipientEmail, subject, htmlContent, 'EyeSentry <no-reply@eyesentry.com>');
            }
        }
        catch (error) {

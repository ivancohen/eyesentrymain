-- ADMIN SYSTEM REBUILD SETUP (SIMPLIFIED VERSION)
-- This script uses a simple approach that works with any table structure
...
-- 3. CREATE A SIMPLE VIEW THAT WORKS WITH ANY SCHEMA
CREATE OR REPLACE VIEW admin_patient_view AS
SELECT 
  pr.id,
  pr.created_at,
  -- Include all basic fields from the response column, regardless of how they're stored
  COALESCE(pr.response::json->>'age', 'Unknown') as age,
  COALESCE(pr.response::json->>'race', 'Unknown') as race,
  -- Include all doctor info
  doc.id as doctor_id,

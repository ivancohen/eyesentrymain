-- ADMIN SYSTEM REBUILD SETUP (FIXED VERSION)
...
-- Check response column type and create appropriate view
DO $$
DECLARE
    column_type TEXT;
BEGIN
    -- Check the data type of the response column
    ...
    -- Log the column type
    RAISE NOTICE 'The response column is of type: %', column_type;
    
    -- Drop the view if it exists
    EXECUTE 'DROP VIEW IF EXISTS admin_patient_view';
    
    -- Create the appropriate view based on column type
    ...
    ELSIF column_type = 'json' THEN
        ...
        LEFT JOIN 
          profiles doc ON pr.doctor_id = doc.id';
    ELSE
        -- For text or other types, try to cast to JSON first
        EXECUTE '
        CREATE VIEW admin_patient_view AS
        SELECT 
          pr.id,

...
CREATE OR REPLACE FUNCTION public.check_doctor_approval_status(p_user_id UUID)
RETURNS JSONB AS $$
DECLARE
  v_status TEXT;
  v_is_doctor BOOLEAN;
...
-- Add comment
COMMENT ON FUNCTION public.check_doctor_approval_status IS 
'Checks if a user is a doctor and their approval status (pending, approved, rejected)';

-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.check_doctor_approval_status TO authenticated;
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.notify_admins_new_doctor TO authenticated;

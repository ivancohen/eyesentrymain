-- ADMIN SYSTEM REBUILD SETUP (FINAL VERSION)
-- This script sets up the database structure for the new admin system
-- And handles all known column type issues
...
-- 3. INSPECT THE TABLE STRUCTURE TO CREATE APPROPRIATE VIEW
DO $$
DECLARE
...
BEGIN
    ...
    SELECT data_type INTO response_type
    FROM information_schema.columns
    WHERE table_name = 'patient_responses'
    ...
    -- Build the view SQL based on the table structure
    view_sql := '
    ...
    -- Handle response JSON fields
    IF response_type = 'jsonb' THEN
        view_sql := view_sql || '
      pr.response->>''age'' as age,
      pr.response->>''race'' as race,';
        
        -- Add risk_level and total_score fields
      ...
      pr.total_score,';
        ELSE
            view_sql := view_sql || '
      pr.response->>''risk_level'' as risk_level,
      (pr.response->>''total_score'')::integer as total_score,';
        END IF;
    ...
    ELSIF response_type = 'json' THEN
      ...
      (pr.response->>''vertical_ratio'')::boolean as vertical_ratio,';
    ELSE
        -- For other types, try to cast to JSON
        view_sql := view_sql || '
      (response::json)->>''age'' as age,
      (response::json)->>''race'' as race,';
      ...
      pr.total_score,';
        ELSE
            view_sql := view_sql || '
      (response::json)->>''risk_level'' as risk_level,
      ((response::json)->>''total_score'')::integer as total_score,';
        END IF;
        
        -- Add risk factors
        view_sql := view_sql || '
      -- All risk factors
      ((response::json)->>''systemic_steroid'')::boolean as systemic_steroid,
      ((response::json)->>''ocular_steroid'')::boolean as ocular_steroid,
      ((response::json)->>''intravitreal'')::boolean as intravitreal,
      ((response::json)->>''family_glaucoma'')::boolean as family_glaucoma,
      ((response::json)->>''iop_baseline'')::boolean as iop_baseline,
      ((response::json)->>''vertical_asymmetry'')::boolean as vertical_asymmetry,
      ((response::json)->>''vertical_ratio'')::boolean as vertical_ratio,';
    END IF;
    
    -- Complete the view with doctor join
    view_sql := view_sql || '
    ...
    -- Execute the finalized SQL
    EXECUTE view_sql;
    
    RAISE NOTICE 'Created admin_patient_view based on the detected schema';
    RAISE NOTICE 'Response column type: %', response_type;
    RAISE NOTICE 'Risk level column exists: %', column_exists;
END $$;

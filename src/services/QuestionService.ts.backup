...
export class QuestionService {
  ...
  static async createDropdownOption(option: Partial<DropdownOption>): Promise<DropdownOption> {
    // Remove display_order if it exists to prevent errors
    const { display_order, ...safeOption } = option;
  ...
  static async updateDropdownOption(id: string, updates: Partial<DropdownOption>): Promise<DropdownOption> {
    // Remove display_order if it exists to prevent errors
    const { display_order, ...safeUpdates } = updates;
  ...
  static async saveDropdownOption(optionData: any) {
    // Remove display_order if it exists to prevent errors
    const { display_order, ...safeOptionData } = optionData;
    
    // If the option has an ID, update it; otherwise, create a new one
    if (safeOptionData.id) {
      return this.updateDropdownOption(safeOptionData.id, safeOptionData);
    } else {
      return this.createDropdownOption(safeOptionData);
    }
  }
